...
//validar el jaque y enroque
    void obtener_posicion_rey(pieza tab[8][8], int color, int *rey_f, int *rey_c);
...
int main(void){
    ...
    while(!WindowShouldClose()){
        ...
        BeginDrawing();
            ...
            //pinta de rojo si el rey esta en jaque
            if (esta_en_jaque(tab, turno_actual)){
                int rf, rc;
                obtener_posicion_rey(tab, turno_actual, &rf, &rc);
                DrawRectangle(rc * TAM_CASILLA, rf * TAM_CASILLA, TAM_CASILLA, TAM_CASILLA, (Color){255, 0, 0, 120});
            }
...
int vltr_rey(pieza tab[8][8], int org_f, int org_c, int chg_f, int chg_c){
    int dif_f = chg_f - org_f;
    int dif_c = chg_c - org_c;
    int enemigo = (tab[org_f][org_c].color == 0) ? 1 : 0;
    //1. intento de enroque 
    if (dif_f == 0 && abs(dif_c) == 2){
        return vltr_enroque(tab, org_f,org_c, chg_f, chg_c);
    }
    //2. movimiento normal de un paso
    if (dif_f == 0 && dif_c == 0) return 1; // no se movio
    if (abs(dif_f) > 1 || abs(dif_c) > 1) return 1; //mas de una casilla

    //no puede pasar por una pieza del mismo color
    if (tab[chg_f][chg_c].sbl != '_' && tab[org_f][org_c].color == tab[chg_f][chg_c].color){
        return 1;
    }

    //el rey no puede moverse a una casilla amenazada
    if (casilla_atacada(tab, chg_f, chg_c, enemigo)){
        return 1;
    }
...
//validar jaque
//encontrar donde esta el rey
void obtener_posicion_rey(pieza tab[8][8], int color, int *rey_f, int *rey_c){
    char objetivo = (color == 0) ? 'k': 'K';
    for (int f = 0; f < 8; f++){
        for (int c = 0; c < 8; c++){
            if (tab[f][c].sbl == objetivo){
                *rey_f = f;
                *rey_c = c;
                return;
            }
...
//valida casilla atacada
int casilla_atacada(pieza tab[8][8], int f, int c, int color_atacante){
    for (int i = 0; i < 8; i++){
        for (int j = 0; j < 8; j++){
            if (tab[i][j].sbl != '_' && tab[i][j].color == color_atacante){
                char s =tab[i][j].sbl;
                int dif_f = f - i;
                int dif_c = c - j;
                //peon atacando casilla
                if (s == 'p' || s == 'P'){
                    int dir = (color_atacante == 0)? -1 :1;
                    if(dif_f == dir && (dif_c == -1 ||dif_c == 1)) return 1;
                }
                else if(s == 'k' || s == 'K'){
                    if(abs(dif_f) <= 1 && abs(dif_c) <= 1 && !(dif_f == 0 && dif_c == 0)){
                        return 1;
                    }
                }
...
//devuelve 1 si el rey de "color" esta en jaque
int esta_en_jaque(pieza tab[8][8], int color){
    int rf, rc;
    obtener_posicion_rey(tab, color, &rf, &rc);
    int color_enemigo = (color == 0)? 1 : 0;
    return casilla_atacada(tab, rf, rc, color_enemigo);
}
...
int vltr_enroque(pieza tab[8][8], int org_f, int org_c, int chg_f, int chg_c){
    int dif_f = chg_f - org_f;
    int dif_c = chg_c - org_c;
    int color = tab[org_f][org_c].color;
    int enemigo = (color == 0) ? 1: 0;

    //solo es intento de enroque si avanza 2 casillas en horizontal en su fila inicial
    if (dif_f != 0 || abs(dif_c) != 2 || tab[org_f][org_c].move != 0){
        return 1;//invalido
    }

    //no se puede enrocar cuando esta en jaque
    if(esta_en_jaque(tab, color)) return 1;

    //enroque corto(hacia columna 6)
    if (dif_c == 2){
        //la torre de ese lado esta en la columna 7
        if (tab[org_f][7].move != 0 || tab[org_f][7].sbl == '_') return 1;
        //casillas intermedias casilas
        if (tab[org_f][5].sbl != '_' || tab[org_f][6].sbl != '_') return 1;
        //el rey no puede cruzar ni esta bajo ataque
        if (casilla_atacada(tab, org_f, 5, enemigo) || casilla_atacada(tab, org_f, 6, enemigo)) return 1;
        return 0;
    }

    //enroque largo
    if (dif_c == -2){
        //la torre de ese lado esta en la columna 0
        if (tab[org_f][0].move != 0 || tab[org_f][0].sbl == '_') return 1;
        //casillas intermedias vacias
        if (tab[org_f][1].sbl != '_' || tab[org_f][2].sbl != '_' || tab[org_f][3].sbl != '_') return 1;
        //el rey no puede cruzar ni estar bajo ataque 
        if (casilla_atacada(tab, org_f, 3, enemigo) || casilla_atacada(tab, org_f, 2, enemigo)) return 1;
        return 0;
    }
    return 1;
}